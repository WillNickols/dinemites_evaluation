...
saveRDS(results, 'real_data_outputs/models_mali_bayesian-drop-out.RDS')
...
write.table(estimated_new_infections_bayesian, 'real_data_outputs/mali_infections_bayesian-drop-out.tsv', sep = '\t', row.names = T)

final_df <- rbind(cbind(dataset_not_for_fitting, probability_new = NA), dataset_for_fitting_bayesian)
write.table(final_df, 'real_data_outputs/mali_probabilities_bayesian-drop-out.tsv', sep = '\t', row.names = F)
